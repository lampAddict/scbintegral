<?php
/* @var $this SiteController */

$this->pageTitle=Yii::app()->name . ' - Цены';
$this->breadcrumbs=array(
    'Цены',
);
?>
<p>
    Цена изделия зависит от общего количества заказанных изделий, ниже приведены ссылки на соответстующие прайс-листы.
</p>
<h4 class="tcolor mtop05em bottom025em">Цена устройста водоограничения</h4>
<p style="margin-bottom: 5px">
    <ul>
        <li>
            <a href="files/prices/uvo1-299.pdf" target="_blank">Цена УВО (ХВС и ГВС) при заказе от 1 до 299 изделий</a>
        </li>
        <li>
            <a href="files/prices/uvo300-999.pdf" target="_blank">Цена УВО (ХВС и ГВС) при заказе от 300 до 999 изделий</a>
        </li>
        <li>
            <a href="files/prices/uvo1000-4999.pdf" target="_blank">Цена УВО (ХВС и ГВС) при заказе от 1000 до 4999 изделий</a>
        </li>
        <li>
            <a href="files/prices/uvo5000.pdf" target="_blank">Цена УВО (ХВС и ГВС) при заказе от 5000 изделий</a>
        </li>
    </ul>
</p>
<h4 class="tcolor mtop05em bottom025em">Цена устройста токоограничения</h4>
<p style="margin-bottom: 5px">
    <ul>
        <li>
            <a href="files/prices/uto1-299.pdf" target="_blank">Цена УТО при заказе от 1 до 299 изделий</a>
        </li>
        <li>
            <a href="files/prices/uto300-999.pdf" target="_blank">Цена УТО при заказе от 300 до 999 изделий</a>
        </li>
        <li>
            <a href="files/prices/uto1000-4999.pdf" target="_blank">Цена УТО при заказе от 1000 до 4999 изделий</a>
        </li>
        <li>
            <a href="files/prices/uto5000.pdf" target="_blank">Цена УТО при заказе от 5000 изделий</a>
        </li>
    </ul>
</p>
